...
"""
PREMIUM JXE ACCOUNT GENERATOR - OB53 (ULTRA SPEED EDITION 2.0 - OPTIMIZED)
...
BASE_FOLDER = os.path.join(CURRENT_DIR, "JXE_GUE_GEN")
...
def ultra_premium_loading_screen():
    ...
    print()
    success_msg = f"{Fore.LIGHTGREEN_EX}★ JXE ENGINE ACTIVATED SUCCESSFULLY ★{R}"
...
def main_menu():
    ...
    while True:
        clear_screen()
        animated_banner()
        title = f"{TIT}JAHID X EMPIRE GUEST GEN{R}"
        options = [
...
def about_section():
    ...
    lines = [
        f" {LBL_BLUE}Name       :{R} {VAL}JXE Account Generator{R}",
        f" {LBL_CYAN}Version    :{R} {VAL}OB54 Premium Ultra Speed{R}",
        f" {LBL_BLUE}Creator    :{R} {VAL}JAHID X EMPIRE{R}",
        f" {LBL_CYAN}Telegram   :{R} {VAL}@Jahid_x_Empire{R}",
...
def safe_exit(signum=None, frame=None):
    global EXIT_FLAG
    EXIT_FLAG = True
    print(f"\n{TIT}👋 Thank you for using JXE Premium Generator{R}")
